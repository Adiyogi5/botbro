...
class CommissionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $my_commission = UserReferral::select('user_referrals.*', 'users.id as user_id', 'users.name', 'users.mobile', 'users.is_approved', 'users.status')
        ...
        return view('frontend.dashboard.commission_history', compact('title', 'user', 'my_commission', 'my_balance', 'requestAmount', 'rejectAmount', 'my_withdrow_request', 'approvedMemberships'));
    }

    public function withdrowcommissionrequest(Request $request)
    {
        $user        = auth('web')->user();
        ...
        try {
            ...
            if ($approvedMemberships < 5 || $totalMembers < 6) {
                ...
                return redirect()->back()->with('error', $errorMessage);
            } else if ($request->amount <= ($balanceData->user_commission_balance - $requestData->sum('request_amount'))) {
                ...
                $availableBalance = $balanceData->user_commission_balance - $requestData->sum('amount');
